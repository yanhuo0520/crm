<template>
  <div class="neayBy">
    ...
    <div class="addressCont">
      ...
      <div class="contCont" v-if="ifShow" @scroll="scrollBottomNeayBy">
...
export default {
  ...
  mounted() {
    ...
    window.addEventListener("scroll", this.scrollBottomNeayBy);
              ...
              geoc.getLocation(pt, function(rs) {
                if(rs.surroundingPois.length > 0){ // 附近的POI点，默认取第一个值
                  ...
                  if(rs.surroundingPois[0].address.length > 1){ // eg:地铁站address为' '
                    ...
                    that.locationdata.address = rs.surroundingPois[0].title;
                  }
                }else{
  ...
  destroyed() {
      window.removeEventListener('scroll', this.scrollBottomNeayBy);//监听页面滚动事件
    }
};
</script>
<style lang="less">
