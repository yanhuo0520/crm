...
export default {
  ...
  mounted(){
    window.addEventListener('scroll', this.scrollBottomSiteSearch)
  },
  methods: {
    changeInput(){
      this.ifShow = false
    },
    scrollBottomSiteSearch(e){
        let that = this;
  ...
  destroyed() {
      window.removeEventListener('scroll', this.scrollBottomSiteSearch);//监听页面滚动事件
    }
