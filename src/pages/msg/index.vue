<template>
  <div class="msg">
    ...
    <ul class="msgList">
      ...
      <!-- 移交客户消息 -->
      <li @click="toDetail('3','我的消息')">
        <img src="../../images/transfa.png" alt="">
        <p>
          <span>我的消息</span><br>
          <span>{{mymessage}}</span>
        </p>
        <img src="../../images/jiantou.png" alt="">
        <div class="van-info van-info--dot dotDiv" v-if="ifMessageNew"></div>
      </li>
      <!-- <li>
...
export default {
  ...
  mounted(){
    ...
    // 我的消息
    this.$api.messageList({type: 3}).then(res=>{
      if(res.data){
        this.mymessage = res.data[0].title
      }else{
        this.mymessage = '暂无通知'
      }
    })
    ...
    this.$api.userInfo().then(res=>{
      ...
      if(res.errno === 0){
        ...
        if(res.data.my_kan === 0){
          this.ifMessageNew = true
        }
      }
    })
  },
