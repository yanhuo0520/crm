  ...
  <div class="work">
    ...
    <div class="waiqin" v-show="curr==0">
      ...
      <template v-if="!locOff && !locWorking && !locPermission && getDistance <= 1000">
        <div class="xianchangSign" :class="{'ifRed': ifRed}" @click="requestCamera()">
    ...
    <div class="image-preview" v-if="ifPreview">
      <van-image-preview v-model="ifPreview" :images="thumbListPre"  @close="onClose()">
        <template v-slot:index>{{pageIn}} / {{thumbListPre.length}}</template>
  ...
  export default {
    ...
    methods: {
      ...
      imgShow(url) {
        ...
        this.thumbListPre = r
        this.downUrl = this.thumbListPre[0]
        // this.thumbListPre = ["http://crm.xfd365.com/public/uploads/20210628/42f765c570ee3e2c093414f79a50e6a6.png"]
      ...
      onChange(index) {
        this.downUrl = this.thumbListPre[index].url
        this.pageIn = index + 1
      },
      downLoadHandle() {
        if (typeof android != 'undefined') {
          android.saveToPhone(this.downUrl);
          this.$toast({
            message: '保存成功',
            icon: 'success'
          })
