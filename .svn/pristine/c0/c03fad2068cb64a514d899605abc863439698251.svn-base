<template>
  <div class="dailyAdd">
    ...
    <div class="content">
      ...
      <div class="marBottom">
        <van-field placeholder="请输入拜访多少户业主" type="number" label="拜访业主数"
          left-icon="http://sc.xfd365.com/crmImages/yezhu.png" input-align="right" v-model="yezhuNum"
          @input="blurInput(yezhuNum, '拜访业主数')" />
        <van-field placeholder="请输入拜访银行客户数" type="number" label="拜访银行客户数" class="label8"
          left-icon="https://ljj-dom.oss-cn-beijing.aliyuncs.com/xfd/crm/bank.png" input-align="right" v-model="bankNum"
          @input="blurInput(bankNum, '拜访银行客户数')" />
        ...
        <van-field placeholder="请输入电话联系多少人" type="number" label="电话联系数"
          left-icon="https://ljj-dom.oss-cn-beijing.aliyuncs.com/xfd/crm/tel.png" input-align="right" v-model="contactNum"
          @input="blurInput(contactNum, '电话联系数')" />
        <van-field placeholder="请输入收集了多少办卡资料" type="number" label="收集办卡资料数" class="label8"
          left-icon="https://ljj-dom.oss-cn-beijing.aliyuncs.com/xfd/crm/ziliao.png" input-align="right" v-model="applyCardNum"
          @input="blurInput(applyCardNum, '收集办卡资料数')" />
  ...
  export default {
    ...
    methods: {
      ...
      saveInfo() {
        ...
        data.customer = this.yezhuNum;
        data.tel_num = this.contactNum;
        data.card_information = this.applyCardNum;
        ...
        // data.yezhuNum = this.yezhuNum;
        // data.customerNum = this.customerNum;
        // data.contactNum = this.contactNum;
        // data.applyCardNum = this.applyCardNum;
        if (this.yezhuNum < 0) {
          this.$toast("拜访业主数不能小于0！");
          return;
        }
        if (this.bankNum < 0) {
          this.$toast("拜访银行客户数不能小于0！");
          return;
        }
        ...
        if (this.contactNum < 0) {
          this.$toast("电话联系数不能小于0！");
          return;
        }
        if (this.applyCardNum < 0) {
          this.$toast("收集办卡资料数不能小于0！");
          return;
        }
        ...
        if (this.$route.query.report_id) {
          ...
          this.$api.reportDetail({
            ...
            if (res.errno === 0) {
              ...
              this.yezhuNum = res.data.customer;
              this.contactNum = res.data.tel_num;
              this.applyCardNum = res.data.card_information;
