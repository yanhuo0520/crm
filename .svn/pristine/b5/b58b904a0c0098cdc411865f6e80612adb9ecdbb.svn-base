  ...
  <div class="weeklyAdd">
    <div class="top">
      ...
      <ul>
        ...
        <li>
          <span>{{reportData.tel_num ? reportData.tel_num : 0}}</span>
          <span>拨打电话</span>
        </li>
        <li>
          <span>{{reportData.customer ? reportData.customer : 0}}</span>
          <span>拜访业主</span>
        </li>
        <li>
          <span>{{reportData.bank ? reportData.bank : 0}}</span>
          <span>拜访银行</span>
        </li>
        <li>
          <span>{{reportData.activity ? reportData.activity : 0}}</span>
          <span>组织活动</span>
        </li>
      </ul>
    </div>
    <div class="content">
      <div class="marBottom">
        <van-cell class="cellDate" title="报告时间" :value="date" is-link @click="toDateShow()"
          icon="http://sc.xfd365.com/crmImages/genjinTime.png"></van-cell>
        <van-calendar v-model="dateShow" type="range" title="报告时间" :default-date="defaultDate" :min-date="minDate"
          :max-date="maxDate" color="#1c92ff" @confirm="onConfirm" confirm-disabled-text="请选择结束时间" />
      ...
      <div class="marBottom">
        ...
        <van-field rows="3" :autosize=autosize type="textarea" maxlength="5000" placeholder="请输入今日总结内容" show-word-limit
          v-model="summaryCont" />
      </div>
      <div class="marBottom">
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      var MondayTime = (day == 0) ? (nowTime - (7 - 1) * oneDayTime) : (nowTime - (day - 1) * oneDayTime); //显示周一
      var SundayTime = (day == 0) ? nowTime : (nowTime + (7 - day) * oneDayTime); //显示周日
      this.start_time = new Date(MondayTime).getFullYear() + '/' + (new Date(MondayTime).getMonth() + 1) + '/' +
        new Date(MondayTime).getDate()
      this.end_time = new Date(SundayTime).getFullYear() + '/' + (new Date(SundayTime).getMonth() + 1) + '/' + new Date(
        SundayTime).getDate()
    ...
    methods: {
      ...
      saveInfo() {
        ...
        //   return;
        // }
        data.tel_num = this.reportData.tel_num;
        data.customer = this.reportData.customer;
        data.bank = this.reportData.bank;
        data.activity = this.reportData.activity;
        // data.card_information = this.reportData.card_information;
    ...
    activated() {
      ...
      if (!this.$route.meta.isBack) {
        ...
        this.start_time = new Date(MondayTime).getFullYear() + '/' + (new Date(MondayTime).getMonth() + 1) + '/' +
          new Date(MondayTime).getDate()
        this.end_time = new Date(SundayTime).getFullYear() + '/' + (new Date(SundayTime).getMonth() + 1) + '/' +
        ...
        if (this.$route.query.report_id) { // 编辑周报
          ...
          this.$api.reportDetail({
            ...
            this.$toast.clear()
            if (res.errno === 0) {
              this.start_time = res.data.start_time.substring(0, 10)
              this.end_time = res.data.end_time.substring(0, 10)
              this.date = this.start_time + ' - ' + this.end_time
              this.defaultDate = [new Date(this.start_time), new Date(this.end_time)]
