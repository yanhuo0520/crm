<template>
  <div class="genjinSel">
    <van-nav-bar left-text="客户跟进" left-arrow fixed @click-left="leftBack" />
    <div class="marBottom">
      <van-cell title="客户名称" :value="customerName" icon="http://sc.xfd365.com/crmImages/name.png" is-link
        @click="contactCcbSel" />
    </div>
    <div class="marBottom">
      <van-cell title="跟进类型" :value="typeName" icon="http://sc.xfd365.com/crmImages/genjinType.png" is-link
        @click="isType=true" />
      <!-- 时间修改为不可选择状态，默认当前时间 -->
      ...
      <van-cell class="getAddress" title="跟进地点" :value="address" icon="http://sc.xfd365.com/crmImages/dingwei.png"
        is-link @click="mapSearch" />
      <baidu-map @ready="handler"></baidu-map>
    </div>
    <div class="marBottom" v-if="typeName == '到访'">
    ...
    <van-action-sheet v-model="isType" title="跟进类型" :actions="actions" @select="onSelect" :round="isRound">
    </van-action-sheet>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      saveInfo() {
        ...
        if (this.cls == 2) { // 建行
          ...
          data.station_id = this.customer_id;
          data.customer_id = 0
        }
    ...
    beforeRouteEnter(to, from, next) {
      ...
      if (from.path == "/contactCcbSel" || from.path == "/siteAdd") {
        to.meta.isBack = true;
      } else {
