    ...
    <div class="searchRust" id="searchRust">
      <ul v-for="(item, index) in lists" :key="index">
        ...
        >
          ...
          <p class="address">{{item.address ? item.address : (initAddr ? initAddr : '未知地址(请在搜索框中重新搜索)')}}</p>
        </li>
      </ul>
    </div>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      cls: '',
      keyword: '',
      initAddr: '' , // 若此值存在 仅仅用于未获取当前定位详情时 地址回显(目前只在addCustomer新增客户页存储)
    };
  },
  mounted() {
    ...
    window.vue = this;
    this.initAddr = localStorage.getItem('initAddrToMap')
    ...
    onClickRight() {
      if (!this.locationdata.lng && !this.locationdata.lat) {
        ...
        this.$router.push({
          path: "/addCustomer",
          query: {
            addressSel: this.locationdata.address ? this.locationdata.address : this.initAddr,
            lnglatSel: this.locationdata.lng + "," + this.locationdata.lat
          }
        });
