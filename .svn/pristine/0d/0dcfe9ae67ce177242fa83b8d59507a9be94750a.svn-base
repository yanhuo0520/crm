<template>
  <div class="tagCustomerSel">
    ...
    <img src="../images/showBank.png" alt="" class="bankSel" v-if='region == "全部" && regionLoad' @click="regionShow = true" />

    <van-popup v-model="regionShow" position="left" :duration="0.3" :style="{ height: '100%',width: '70%' }">
      <ul v-if="secondBank != ''" class="secondBankUl">
        <li v-for="(item, index) in secondBank" :class="{'active': index == activeIndex}" :key="index" @click="secondBankSel(index, item)">{{item}}</li>
      </ul>
    </van-popup>
    <img src="../images/hideBank.png" alt="" class="bankHide" v-if='region == "全部" && regionShow' @click="regionShow = false" />
    ...
    <div v-if="userData != ''">
      <van-index-bar :index-list="indexList" :sticky-offset-top="stickyOffsetTop">
        ...
        <!-- <van-checkbox-group v-model="result" @change="changeSel">
...
export default {
  ...
  methods: {
    ...
    getsecondBank(){
      this.$api.secondBank().then(res => {
        if (res.errno === 0){
          if(res.data){
            this.secondBanksel = res.data[0]
            this.getList();
          }
          this.secondBank = res.data
        }
      })
    },
    secondBankSel(index, item){
      // console.log(index + '--' + item)
      this.regionShow = false
      this.activeIndex = index
      this.secondBanksel = item
      // 清空数据，重新加载
      this.loading = 1
      this.searchInput = ''
      this.indexList = []
    ...
    changeSel(){
      let that = this;
      that.selNum = this.result.length
      if(that.selNum > 0){
      ...
      that.result.forEach(ele=>{
        if(ele.indexOf("station") > -1){
          stationIds += ele.slice(0, ele.indexOf("station")-1) + ','
        }else if(ele.indexOf("customer") > -1){
          customerIds += ele.slice(0, ele.indexOf("customer")-1) + ','
        }
      })
      if(stationIds.lastIndexOf(',') > -1){
        that.stationIds = stationIds.substr(0,stationIds.length-1)
      }else{
        that.stationIds = stationIds
      }
      if(customerIds.lastIndexOf(',') > -1){
        that.customerIds = customerIds.substr(0,customerIds.length-1)
      }else{
