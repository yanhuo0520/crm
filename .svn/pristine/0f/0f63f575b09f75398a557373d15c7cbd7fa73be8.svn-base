<template>
  <div class="chaiList">
    <van-nav-bar left-text="月度统计列表" left-arrow fixed @click-left="leftBack" >
      <template #right>
    ...
    <div class="content" v-if="lists!=''">
      <div class="listUl" v-for="(item, index) in lists" :key="index">
        ...
        <ul>
          ...
          <li @click="edit(item.travel_id)">编辑</li>
        </ul>
      </div>
    </div>

    <div v-else-if="lists == '' && keyword" class="zanwu">
      <span>暂无<span class="tip">"{{keyword}}"</span>的费用</span>
    </div>
  ...
  export default {
    ...
    methods: {
      ...
      handleTimeDown(value){
        ...
        window.location.href = 'http://crm.xfd365.com/user/report/export?user_id=' + this.userId + '&date=' + datetime + '&token=' + this.token
